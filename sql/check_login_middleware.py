...
class CheckLoginMiddleware(object):
    def process_request(self, request):
        ...
        if not remoteIp.startswith('192.168') and not remoteIp.startswith('10.') and request.path not in ('/login/','/logout/', '/authenticate/','/allworkflow/') and not re.match(r'^/detail/(?P<workflowId>[0-9]+)/$',request.path) and not re.match(r'^/execute/$',request.path) and not re.match(r'^/cancel/$',request.path):
